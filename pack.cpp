...
map<string, int> fly_config_keyword_map;
...
string fly_make_real_url(const string& url)
{
	...
	if((n = url.find(fly_domain, 0)) != string::npos)//url中包含域名，要将其删去
		url2 = url.substr(fly_domain.size(), url.size() - fly_domain.size());
	else
		url2 = url;

	if(fly_docroot[fly_docroot.size() - 1] == '/')//配置项docroot末尾有'/'
	{
		if(url2[0] == '/')
			real_url = fly_docroot + url2.erase(0, 1);
		else
			real_url = fly_docroot + url2;
	}
	else//配置项docroot末尾没有'\'
	{
		if(url2[0] == '/')
 ...
 */
 ...
 {
 	fly_config_keyword_map.insert(make_pair("docroot", FLY_DOCROOT));
 	fly_config_keyword_map.insert(make_pair("domain", FLY_DOMAIN));
 }
/*
 *函数作用：解析配置文件
 *函数参数：path为绝对路径+文件名
 ...
 */
 int fly_parse_config(const char *path)
 {
 	...
 	while(getline(infile, line))
 	{
 		...
 		stream >> word;//keyword
 		map<string, int>::const_iterator cit= fly_config_keyword_map.find(word);
 		if(cit == fly_config_keyword_map.end())
 		{
